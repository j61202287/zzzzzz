...
import { keywordTopics, tvGenres, tvNetworks } from "@/constants/filter";
...
export default function DiscoverTv() {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useGetDiscoverInfinite<MovieTypes>({
      ...
      params: {
        ...
        ...(yearType === true &&
          toValue != null && { "first_air_date.gte": `${toValue}-01-01` }),

        ...(yearType === true &&
          fromValue != null && { "first_air_date.lte": `${fromValue}-12-31` }),
        ...
        ...(selectedNetwork && { with_networks: selectedNetwork }),
  ...
  const selectedNetworkLabel = tvNetworks.find(
    (network) => network.id === selectedNetwork,
  )?.name;
  //FILTERS
  const selectedGenreLabels = tvGenres.filter((genre) =>
  ...
  return (
    <div className=" py-15 space-y-2 mx-auto lg:w-[85%] w-[95%] ">
            ...
            <div className="overflow-auto custom-scrollbar space-y-3">
              <div className="lg:p-4 p-2 lg:space-y-3 space-y-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {tvGenres
                    .slice(0, expandGenre ? tvGenres.length : 4)
                    .map((genre) => (
                      <Button
                        key={genre.id}
                        size="xl"
              ...
              <div className=" lg:p-4 p-2 lg:space-y-3 space-y-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {tvNetworks
                    .slice(0, expandCompanies ? tvNetworks.length : 4)
                    .map((network) => (
                      <Button
                        variant={
      ...
      <div className="grid lg:grid-cols-7 md:grid-cols-5 sm:grid-cols-4 grid-cols-2 lg:gap-4 gap-2">
